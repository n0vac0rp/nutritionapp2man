...
import { getDailyCalorieRecommendation } from "../utils/calculations"
import { toDateKey } from "../utils/dates"
import { Calendar, TrendingUp, Clock, BarChart3, PieChart } from "lucide-react"
...
export default function MonthlyAnalysis() {
  ...
  const startDate = toDateKey(new Date(selectedYear, selectedMonth, 1))
  const endDate = toDateKey(new Date(selectedYear, selectedMonth + 1, 0))
  ...
  const processWeeklyData = (): WeekData[] => {
    ...
    while (currentWeekStart <= lastDay) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const dateString = toDateKey(currentDay)
                          ...
                          <div key={day.date} className="border rounded p-3 text-center">
                            ...
                            <div className="flex justify-center gap-1 mt-2">
                              {["breakfast", "lunch", "dinner", "snack"].map((mealType) => {
                                const hasMeal = day.meals.some((meal) => meal.type.toLowerCase() === mealType)
                                return (
                                  <div
                                    key={mealType}
                                    className={`w-2 h-2 rounded-full ${hasMeal ? "bg-green-500" : "bg-muted"}`}
                                    title={mealType}
                                  />
                                )
                              })}
